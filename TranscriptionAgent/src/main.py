...
async def transcribe_endpoint(request: TranscribeRequest):
    ...
    logger.info(f"Transcribing audio_url: {audio_url}, metadata: {request.metadata}")
    ...
    if(ENV_MODE == "local"):
        ...
        p = Path(audio_url)
        transcibe_file_path = str(p.with_name(f"{p.stem}_transcribe.txt"))
        ...
        transcibe_filename = f"{name_without_ext}_transcribe.txt"
    ...
    try:
        ...
        transcript_text = transcript_extraction(audio_url_path, transcibe_file_path)
    except Exception as e:
    ...
    return {
        ...
        "transcibe_url": transcibe_url,
        "metadata": { # Additional information to be returned if any
            "ENV_MODE" : ENV_MODE
        }
    }
    
def transcript_extraction(audio_url_path, transcibe_file_path):
    ...
    try:
        ...
        result = groq_service.process_incident(audio_url_path, task_type="transcribe")
