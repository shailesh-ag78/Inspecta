...
async def get_incidents_for_site(
    siteId: int = Query(..., description="Site ID"),
    companyId: int = Query(..., description="Company ID")
):
    """Fetch incidents for a given site"""
    print(f"Received GET /api/incidents request for siteId {siteId} and companyId {companyId}")
    try:
        incidents = await repository.get_incident_for_site(siteId, companyId)
        print(f"✅ Fetched {len(incidents)} incidents for siteId {siteId}")
...
#     """Fetch all sites"""
#     try:
#         # Create a temporary repository to get sites
#         # Note: sites don't require company_id context for initial fetch
#         sites_query = """
#             SELECT 
#               s.id, 
#               s.site_name as name, 
...
#         # Use repository's getAllSites method if available
#         # For now, we'll return the data from the repository method
#         sites = await repository.getAllSites()
