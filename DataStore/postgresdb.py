...
class IncidentRepository:
    ...
    async def get_incident_for_site(self, site_id: int, company_id: int) -> List[Dict]:
        """Fetches all incidents for a specific site and company, filtered by RLS."""
        print(f"Fetching incidents for site_id={site_id} and company_id={company_id}")
        async with self.session(company_id) as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "SELECT * FROM incidents WHERE site_id = %s AND company_id = %s",
                    (site_id, company_id)
                )
                return [dict(row) for row in await cur.fetchall()]